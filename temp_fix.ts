...
async function main() {
  try {
    const startTime = Date.now();
    const verificationService = new BidVerificationService();

    // Input file path
    const inputFile = resolve(
      process.cwd(),
      "public/data/bids_table_20250915_original.json",
    );

    console.log("🚀 Starting bid verification from JSON file...");
    console.log(`📄 Source file: ${inputFile.split("/").pop()}`);

    // Step 1: Load bids from file
    const bids = verificationService.loadBidsFromFile(inputFile);

    // Step 2: Verify all bids
    const results = await verificationService.verifyAllBids(bids);

    // Step 3: Remove duplicates (same bidder + tokenId), keeping only most recent
    const { filteredResults: deduplicatedResults, duplicatesFound, duplicatesRemoved } =
      verificationService.removeDuplicates(results);
    ...
    // Print summary
    console.log("\n🎉 Bid verification completed successfully!");
    console.log(`\n📊 Final Summary:`);
    console.log(`• Total bids checked: ${report.summary.totalBids}`);
    console.log(`• Active bids found: ${report.summary.activeBids}`);
    console.log(`• Duplicates removed: ${report.summary.duplicatesRemoved}`);
    console.log(`• Currency symbols updated: ${report.summary.currencySymbolsUpdated}`);
    console.log(`• Success rate: ${report.summary.successRate}%`);
    console.log(`\n📁 Files saved:`);
    console.log(`• Active bids: ${outputPath}`);
